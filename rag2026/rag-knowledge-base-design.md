...
Databases make terrible file cabinets, and cloud object storage is off the table, because the compliance boundary says everything stays on infrastructure the firm controls. So originals live on **mounted volumes** (AWS EBS behind an NFS export, or the on-prem NAS), and the database stores only a `storage_uri` pointing at each file. The database knows *where* everything is; it just refuses to carry it.
...
- *"What is our aggregate exposure to commercial real estate in Europe?"* is thematic and conceptual. Keyword search is useless; no document contains that sentence.
- *"Find clause 7.3(b) of the Meridian facility agreement."* is an exact needle. Semantic search is actively dangerous here: it will happily return something *similar* to clause 7.3(b), delivered with total confidence. Similar is worse than nothing.

**Solution zero: make both sides speak the same language.** There's a quiet problem underneath vector search. A question and a contract paragraph are different species: *"What's our CRE exposure in Europe?"* is eight words of informal shorthand; the text that answers it is three hundred words of dense legalese. Embed them as-is and you're not comparing apples to apples; you're comparing apples to legal oranges. So both sides get pre-processed into comparable form *before* any embedding happens:

- **One normalizer, two callers.** A single shared module cleans and canonicalizes text: Unicode and whitespace normalization, acronym expansion from a curated dictionary ("CRE" → "commercial real estate (CRE)"), canonical entity names from the entity index, dates written out in full. It runs on chunks at ingestion and on queries at search time, the *same code path* by design, so the two sides can never drift apart. The dictionary is versioned like the embedding model: extend it, and the affected chunks re-embed through the usual blue/green machinery.
- **Model-specific formatting.** The embedding models in this family (BGE/E5) are trained *asymmetrically*: queries expect a `query:` prefix, passages a `passage:` prefix. It looks like a trivial string constant, and skipping it silently costs several points of recall. The most expensive missing prefix in the business.
- **Chunks arrive dressed for the comparison.** Every chunk already carries its heading path (`Credit Agreement > Section 4.2 > ...`) prepended into the embedded text, so a bare clause fragment or table row brings its context along instead of floating in space.
- **Hard queries get a stunt double.** For broad, abstract questions, an optional **HyDE** step (hypothetical document embeddings) has the LLM draft a short *hypothetical answer* in document style and embeds that instead: a fake document to find the real ones, because a fake document resembles a real document far more than a question ever will. Like the LLM reranker, it's a conditional escalation: adopted only where the golden suite shows it earns its latency.

None of this is assumed to help. Every pre-processing step is toggled and measured against the golden suite, because "common practice" is a hypothesis, not a result.
...
**At retrieval time**, after hybrid search returns its chunks, the engine walks one hop. Retrieve a credit agreement, and its amendments come along, even though they never matched the query. Retrieve something with a `supersedes` edge pointing at it, and the answer gets *flagged as superseded*, structurally, rather than by hoping the newer version happened to rank well. And every hop joins the ACL table first: a link must never reveal so much as the *existence* of a document the user can't see.

**For humans**, the graph renders as an **interconnected wiki**: one Markdown file per document, YAML frontmatter on top (type, dates, classification, status), `[[wikilinks]]` for every relationship. It is browsable in Obsidian, greppable with ripgrep, and laid out in a readable tree (`wiki/documents/<collection>/<type>/<year>/...`, plus a stub page per entity listing everywhere it appears). Compliance officers get to browse the corpus by its actual structure instead of playing search-term roulette.
...
Rule two: **the visibility flip is atomic.** Old chunks off, new chunks on, current-flag moved, all in one transaction. No query ever catches a document half-dressed.
...
- **Insufficient evidence gets an honest refusal:** *"The available documentation does not contain sufficient evidence to answer this inquiry."* The system is graded on saying this at the right moments, which is exactly what the unanswerable set is for. Saying "I don't know" well is a skill, and here it's a tested one.

For questions where being wrong is expensive, the design offers two stronger modes, both built on the same insight: **hallucinations are idiosyncratic, but truths converge.** Two models rarely invent the *same* falsehood; they routinely agree on the same fact.

- **Fusion mode.** Two or three *independent* self-hosted models answer the same question from the same retrieved context, and a judge model synthesizes: claims that multiple models make, with citations that check out, survive; claims that only one model produces get treated as probable hallucinations and dropped or flagged. It's the jury system for language models: no single juror's imagination becomes the verdict. The cost is honest (3-4× inference), so fusion is tiered rather than default. It switches on for sensitive collections, for queries where retrieval confidence is low, and on demand, through a "verify mode" toggle in the user chat for the answer someone is about to put in front of a client.
- **Persona cross-examination.** The cheaper sibling: one model, several perspectives. The same question is answered as a credit officer, then reviewed as a skeptical auditor, then as opposing counsel looking for the weak spot. Where the personas agree, confidence rises; where they diverge, the answer says so, and divergence itself is a signal worth surfacing, because a claim that survives cross-examination from three angles is a claim standing on evidence rather than on style.
...
- Pinned **by digest** rather than by tag (`postgres:16.6@sha256:...`), so the image cannot change underneath you. `:latest` is banned outright.
