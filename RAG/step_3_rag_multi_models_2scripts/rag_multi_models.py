...
# ---------------------------------------------------------------
async def do_rag(query, collection):
    ...
    if collection != "all":
        ...
        for collection_ in m_client.list_collections():
            search_results.append(m_client.search(
...
def get_all_collections():
    ...
    collections = m_client.list_collections()
    collections.append("all")

    options_list = [Option(collection, value=collection) for collection in collections]

    return Div(Label("Select collection to search:"),
                Select(id="table_search", name="collection")(
                            *options_list
                           ), 
                id="collections")
...
#---------------------------------------------------------------
@app.ws('/wscon')
async def ws(data:str, send, model:str, collection:str):
    """ Call Ollama or OpenAI and get responce using streaming """
    global isRAG
    
    print(f"{data}, {model}, {collection}")
    if isRAG:
        context = await do_rag(data, collection)
