...
"""
# test03_chat_openai.py
# Example of using fastapi + openai
...
async def ws(msg: str, send):
    ...
    for chunk in stream:
        if chunk.choices[0].delta.content is not None:
            await send(Span(chunk.choices[0].delta.content, id=f"chat-content-{len(messages)-1}", hx_swap_oob="beforeend") )
